    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-6">
        <div>
            ...
            <p class="text-slate-500 font-medium flex items-center">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"></path></svg>
                {{ $project->url }}
            </p>
        </div>
            ...
            <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/40 sticky top-28">
                ...
                <form wire:submit.prevent="addCompetitor" class="space-y-6">
                    <div class="space-y-2">
                        ...
                        <input type="text" wire:model="comp_name" class="w-full px-5 py-4 bg-slate-50 border-transparent rounded-2xl focus:ring-2 focus:ring-indigo-500 font-semibold" placeholder="e.g. Nike">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" wire:model="comp_url" class="w-full px-5 py-4 bg-slate-50 border-transparent rounded-2xl focus:ring-2 focus:ring-indigo-500 font-semibold" placeholder="https://nike.com">
                            ...
                            <tr class="group hover:bg-slate-50/50 transition">
                                <td class="px-8 py-6">
                                    <div class="flex items-center space-x-4">
                                        <div class="w-10 h-10 bg-white border border-slate-200 rounded-xl flex items-center justify-center font-bold text-indigo-600 shadow-sm">
                                            {{ substr($competitor->name, 0, 1) }}
                                        </div>
                                        <div>
                                            <p class="font-bold text-slate-900">{{ $competitor->name }}</p>
                                            <p class="text-xs text-slate-400 font-medium">{{ $competitor->website_url }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-8 py-6">
                                    <span class="inline-flex items-center px-3 py-1 bg-amber-50 text-amber-600 text-[10px] font-bold uppercase rounded-full tracking-widest border border-amber-100">
                                        Pending Fetch
                                    </span>
                                </td>
                                <td class="px-8 py-6">
                                ...
                                <td class="px-8 py-6 text-right">
                                    <button class="text-sm font-bold text-indigo-600 hover:underline">View Report</button>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="4" class="px-8 py-20 text-center">
                                    <div class="flex flex-col items-center">
                                        <div class="w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center mb-4">
                                            <svg class="w-8 h-8 text-slate-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                        </div>
                                        <p class="text-slate-400 font-medium italic">Your competitor list is empty. Add one to see the magic.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
